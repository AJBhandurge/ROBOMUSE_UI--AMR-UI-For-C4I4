...
def stopMission():
    """Stop the running mission gracefully.

    Steps:
      1. Kill the mission_runner subprocess (stops it from sending new goals).
      2. Cancel the active Nav2 goal SYNCHRONOUSLY — we block until the action
         server confirms the cancel (or times out).  This is critical: if we
         return the HTTP 200 while Nav2 is still in CANCELING state, a
         Goal-to-Point sent immediately afterward will be silently rejected
         because Nav2 won't accept a new goal until it finishes the cancel.
    """
    global mission_process
    kill_process(mission_process)
    mission_process = None

    # Run the cancel synchronously so Nav2 is fully IDLE before we respond.
    # timeout=5 is generous; a clean cancel typically completes in <1 s.
    try:
        subprocess.run(
            ["ros2", "action", "cancel", "/navigate_to_pose"],
            timeout=5,
        ...
        pass   # timeout or ros2 not found — not fatal
