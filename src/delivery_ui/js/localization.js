...
amclPoseTopic.subscribe(function (msg) {
  ...
  if (!poseHasBeenSet) {
    ...
    /* Enable nav buttons now that we have a real position */
  ...
  /* Always persist latest AMCL position — used on refresh to restore pose */
  _saveLastPose(p.x, p.y, yaw);
  ...
  fetch(SERVER_URL + "/start_localization", {
    ...
    .then(function () {
      ...
      if (saved) {
        ...
        robotLayer.x        =  saved.x;
        robotLayer.y        = -saved.y;
        robotLayer.rotation = -saved.yaw * (180 / Math.PI);
        _robotYawRad        =  saved.yaw;
        poseHasBeenSet      = true;
        if (typeof updateRobotPosHUD === "function") updateRobotPosHUD(saved.x, saved.y);
