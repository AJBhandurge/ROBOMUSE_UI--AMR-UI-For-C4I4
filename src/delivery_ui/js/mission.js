...
function stopMission() {
  /* Disable Goal-to-Point immediately so the user cannot fire a goal while
     Nav2 is still processing the cancel (it rejects new goals during CANCELING). */
  var btnGoal = document.getElementById("btn-goal");
  var bst     = document.getElementById("btn-stop-mission");
  if (btnGoal) { btnGoal.disabled = true; btnGoal.classList.remove("btn-goal-active"); }
  ...
  fetch(SERVER_URL + "/mission/stop", { method: "POST" })
    ...
    .then(function () {
      ...
      /* Re-enable Goal-to-Point — server only responds after cancel is confirmed */
      if (btnGoal) btnGoal.disabled = false;
      if (typeof clearNavPath === "function") clearNavPath();   /* remove green path line */
      showToast("⛔ Mission stopped", "info");
    })
    .catch(function () {
      /* On error still re-enable so user isn't stuck */
      if (btnGoal) btnGoal.disabled = false;
      if (bst)     bst.disabled = false;
      showToast("⚠ Failed to stop mission", "error");
    });
}
